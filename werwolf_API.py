...
CORS(app, supports_credentials=True, origins=["https://werwolf-fronted.onrender.com"])
...
def api_names():
    """ Namen verwalten – nur überschreiben, wenn wirklich Listeneintrag mit Namen vorhanden! """
    if request.method == 'GET':
        return jsonify({"names": get_from_session('names', [])})
    else:
        names = request.json.get('names', None)
        # Speichere nur, wenn Liste mit min. 1 nicht-leerem Namen kommt!
        if names is not None and isinstance(names, list) and len([n for n in names if n.strip()]) > 0:
            save_to_session('names', names)
            return jsonify({"message": "Namen gespeichert.", "names": names})
        # NICHT überschreiben, wenn leer!
        return jsonify({"message": "Namen nicht überschrieben, da Input leer.",
